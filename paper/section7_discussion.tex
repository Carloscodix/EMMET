...
Our headline workload is uniformly random source--destination
pairs at fixed step intervals. Real datacenter and ISP traffic
shows heavy-tailed flow sizes, locality, and time-of-day patterns.
We ran exploratory tests with Pareto inter-arrivals and Lognormal
flow sizes (median 3 packets, $\alpha=1.5$) on GÉANT with
matched bursty warmup. Under that regime LASP-aug, CONGA-WAN,
and EMMET-DP converge to within $\pm 4\%$ of each other --- the
per-packet memory mechanism, tuned for Poisson regimes, does
not extract usable signal from short heavy-tailed bursts.
Recalibrating $\kappa$, $\alpha_{\rm budget}$ and the load-decay
parameter $d_\lambda$ for bursty traffic is left to future work;
we suspect a collective-pressure extension (saturated edges
discounting their neighbours) would be the natural direction.
The improvements we document should therefore be read as bounds
on \emph{algorithmic} performance under Poisson-like conditions,
not as predicted production gains.
