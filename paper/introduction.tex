...
forward-progress decisions. LASP~\cite{cohen2014lasp} weights
...
router (\emph{Mass-Aware DP}) over the joint state space
...
DP reduces congestion losses by $60.2\%$ relative to LASP-aug, a
Dijkstra baseline using the same integrated potential function
(paired $t = 5.21$, Wilcoxon $p = 1.1\times 10^{-8}$). The
improvement holds with negligible additional capacity consumption
per delivered packet ($+0.04$ hops on average) and across $22$
scenarios spanning five topology families: Erd\H{o}s--R\'enyi
($14$ configurations), scale-free Barab\'asi--Albert,
...
Section~\ref{sec:algorithm} presents the Mass-Aware DP algorithm
