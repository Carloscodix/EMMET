...
controller, no per-flow state, no precomputed path tables, sixty lines of code. (Sixty lines describe the mechanism, not the operating cost; Section~\ref{sec:control-plane} prices that difference.)
...
One reading note. Sections~\ref{sec:core} and~\ref{sec:ablation} present the model as it was originally conceived and reduced---gradient plus a Newton-III loss reaction---because the re-audit that later \emph{inverts} that verdict (Section~\ref{sec:redux}) is itself one of the results. The reader should know the destination in advance: the core this paper ultimately validates is gradient plus an anticipatory threshold force; the two-factor law of Section~\ref{sec:twofactor} is the central claim; and the physical cores are best understood as instruments. We preserve the chronology because the inversion, and the machinery that caught it, are part of the contribution.
