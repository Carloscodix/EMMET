...
forwarding plane. In particular, micro-bursts---the microsecond-scale queue excursions that DRILL's per-packet reactivity is designed to absorb---do not exist at flow granularity; the equivalence reported against DRILL is therefore established for sustained-flow traffic and is not claimed for the micro-burst regime. Testing it requires a packet-level instrument (an ns-3-class simulator or an emulated data plane), outside the scope of the flow-level harness used here.
...
Two narrower directions emerge from the analysis above but lie beyond, or remain ahead of, the present instrument. An
