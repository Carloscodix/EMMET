"""Generic bootstrap CI 95% + effect sizes pipeline.

Reusable across Phase A (n<=500), Phase B (n=1000), and any future
battery. Same statistical machinery as bootstrap_ci_v2.py and
bootstrap_ci_scalability.py, factored to accept any raw JSON.

Usage:
  python3 bootstrap_ci_generic.py <src.json> <dst.json>

Requires src to be a list of dicts with keys: scenario, seed, kappa,
lasp_aug.losses, momentum_dp.losses (the standard v1 paired format).
"""
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print('Usage: python3 bootstrap_ci_generic.py <src.json> <dst.json>')
        sys.exit(1)
    pretty(main(sys.argv[1], sys.argv[2]))
