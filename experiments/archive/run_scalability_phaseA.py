"""Phase A of scalability v2: launch n in {100, 250, 500} only.
Leaves n=1000 for a second nightly run after we validate Phase A results."""
...
# Filter: keep only n <= 500
all_jobs = battery_jobs()
jobs = [j for j in all_jobs if int(j[2][0]) <= 500]
print(f'Phase A: {len(jobs)} jobs (of {len(all_jobs)} total, dropping n=1000)')

workers = max(1, cpu_count() - 4)
print(f'workers: {workers} (of {cpu_count()} cores)')
sys.stdout.flush()

raw_out = DATA / 'scalability_phaseA_raw.json'
ckpt_out = DATA / 'scalability_phaseA_checkpoint.json'
summary_out = DATA / 'scalability_phaseA_summary.json'
